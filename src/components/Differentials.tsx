...
export default function Differentials() {
  return (
    <section id="diferenciais" className="bg-ink py-20 md:py-28">
      <div className="container-px">
        <Reveal>
          <SectionTitle
            eyebrow="Diferenciais"
            title="Por que escolher a Monica"
            align="center"
          />
        </Reveal>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {differentials.map((item, index) => (
            <Reveal key={item.title} delay={index * 100}>
              <article className="group h-full rounded-2xl border border-white/5 bg-ink-soft p-6 transition-colors hover:border-brand/40">
